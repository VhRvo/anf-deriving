...
module Plain1.Conversion2.Step02 where
...
-- `reifyWithNaturality` states that `applyFrame` can be pushed through
-- `reifyWith`
-- by postcomposing the builder with the surrounding frame.
reifyWithNaturality :: Frame -> (Atom -> AExpr) -> Comp -> Bool
-- `reifyWithNaturality frame k comp = applyFrame frame (reifyWith k comp) == reifyWith (applyFrame frame . k) comp`
reifyWithNaturality frame k comp =
  case comp of
    CAtom atom ->
      begin
        (applyFrame frame (reifyWith k (CAtom atom)))
        =< "by unfolding reifyWith"
        >= (applyFrame frame (k atom))
        =< "by folding reifyWith"
        >= (reifyWith (applyFrame frame . k) (CAtom atom))
        & qed
    nonAtomComp ->
      begin
        ...
        =< "by folding reifyWith"
        >= (reifyWith (applyFrame frame . k) nonAtomComp)
        & qed
      ...
      let
       ...
       in
        begin
          ...
          =< "by reifyWithNaturality and simplifying function composition"
          >= (applyFrame funK1 (conv funExpr))
          =< "by the induction hypothesis on funExpr"
          >= (convK funExpr funK1)
          =< "by applyFrameAssociativity, reifyWithNaturality, and the induction hypothesis on argExpr"
          >= (convK funExpr funK)
          & end
      ...
      let
       ...
       in
        begin
          ...
          >= (convK lhsExpr lhsK1)
          =< "by applyFrameAssociativity, reifyWithNaturality, and the induction hypothesis on rhsExpr"
          >= (convK lhsExpr lhsK)
          & end
